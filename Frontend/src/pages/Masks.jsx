  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Filter products by the "masks" category
        const maskProducts = response.data.filter(product => product.category === 'masks');
        setProducts(maskProducts);
      } catch (error) {
  ...
  return (
    <section id="product1" className="max-w-screen-xl mx-4 text-center py-7 md:mx-8 lg:mx-16">
      <div className="w-full mb-8">
        <img src={cover} alt="Masks Collection Cover" className="w-full h-auto rounded-lg shadow-lg" />
      </div>
      <h2 className="mb-8 text-4xl font-bold">-Traditional Wooden Masks-</h2>
...
export default TraditionalMasks;
