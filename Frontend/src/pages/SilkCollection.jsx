  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Filter products by the "silk" category
        const silkProducts = response.data.filter(product => product.category === 'silkcollection');
        setProducts(silkProducts);
      } catch (error) {
  ...
  return (
    <section id="product1" className="max-w-screen-xl mx-4 text-center py-7 md:mx-8 lg:mx-16">
      <div className="w-full mb-8">
        <img src={cover} alt="Silk Collection Cover" className="w-full h-auto rounded-lg shadow-lg" />
      </div>
      <h2 className="mb-8 text-4xl font-bold">-Silk Collection-</h2>
